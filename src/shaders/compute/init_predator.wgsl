...
fn compute_predator_position(@builtin(global_invocation_id) id: vec3<u32>) {
  ...
  predators[id.x].pos = vec2<f32>(rx.value * 150.0, ry.value * 70.0);
  predators[id.x].vel = vec2<f32>(rvx.value, rvy.value) * 0.2;
}
